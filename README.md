...
FF14 国服石之家社区的 [OpenCLI](https://github.com/jackwener/opencli) 插件，用命令行读取石之家帖子、攻略、真实招募列表/详情、当前登录角色档案、未读消息数量和签到记录。
...
`recruit-guild`、`me`、`notifications` 和 `checkin` 需要浏览器登录态：
...
先用 Chrome 登录石之家网页端，并确认网页里能正常打开「我的」页面。登录后重新运行 `recruit-guild`、`me` 或 `notifications`。

### 为什么 `posts` 能用，但 `me` / `notifications` 不行？

`posts` 和 `recruit` 使用公开接口；`recruit-guild`、`me`、`notifications` 和 `checkin` 读取账号相关信息，需要本机浏览器里已有登录态。
...
- `me`、`notifications` 和 `checkin` 依赖本机 Chrome 登录态，无法在未登录环境中读取账号数据。
