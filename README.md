...
FF14 国服石之家社区的 [OpenCLI](https://github.com/jackwener/opencli) 插件，用命令行读取石之家帖子、攻略、真实招募列表/详情、当前角色统计总览、当前登录角色档案、未读消息数量和签到记录。
...
`recruit-guild`、`statistics`、`me`、`notifications` 和 `checkin` 需要浏览器登录态：
...
`posts` 和 `recruit` 使用公开接口；`recruit-guild`、`statistics`、`me`、`notifications` 和 `checkin` 读取账号相关信息，需要本机浏览器里已有登录态。
